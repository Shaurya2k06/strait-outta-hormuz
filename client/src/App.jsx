...
function VulnerabilityMap({ activeCustomer, setActiveCustomer }) {
  ...
  const max = Math.max(...visibleCustomers.map((customer) => customer.adverse))
  return (
    <article className="panel map-panel"><div className="panel-heading"><div><div className="eyebrow">impact × rate severity</div><h3>Scale does not equal severity</h3></div><span className="panel-tag">click a bubble to focus</span></div><div className="map-legend"><span><i className="legend-dot blue" /> positive adverse sensitivity $</span><span><i className="legend-dot amber" /> sensitivity / shock revenue</span></div><svg aria-label="Customer impact versus rate severity map" className="scatter" role="img" viewBox="0 0 560 280"><line className="axis" x1="48" x2="530" y1="238" y2="238" /><line className="axis" x1="48" x2="48" y1="24" y2="238" /><line className="grid-line" x1="48" x2="530" y1="185" y2="185" /><line className="grid-line" x1="48" x2="530" y1="132" y2="132" /><line className="grid-line" x1="48" x2="530" y1="79" y2="79" /><text className="axis-label" x="48" y="263">0%</text><text className="axis-label" x="470" y="263">400% →</text><text className="axis-label" transform="rotate(-90)" x="-224" y="16">positive adverse $</text>{visibleCustomers.map((customer) => { const x = 55 + Math.min(customer.severity, 400) / 400 * 460; const y = 226 - customer.adverse / max * 185; const radius = 8 + Math.sqrt(customer.contracted / 1000000) * 2.1; const active = activeCustomer === customer.name; const activate = () => setActiveCustomer(active ? '' : customer.name); return <g className={`bubble ${active ? 'active' : ''}`} key={customer.name} onClick={activate} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activate() } }} role="button" tabIndex="0"><circle cx={x} cy={y} r={radius} /><text x={Math.min(x + 10, 460)} y={y - 10}>{shortName(customer.name)}</text></g> })}</svg><div className="map-footer"><span>Bubble size = full contracted revenue</span><span>Axes use raw dollars and aggregate rates</span></div></article>
  )
}
