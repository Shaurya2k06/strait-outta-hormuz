...
function calculateScenario(inputs) {
  const deliveredBaseCost = controls.deliveredCost - controls.deliveredInsurance - controls.deliveredPenalty
  const deliveredCost = deliveredBaseCost * inputs.costMultiplier + controls.deliveredInsurance * inputs.insuranceMultiplier + controls.deliveredPenalty * inputs.penaltyMultiplier
  const recoveredSurcharge = controls.deliveredAdverseSensitivity * inputs.recoveryRate
  const deliveredContribution = controls.recognizedRevenue + recoveredSurcharge - deliveredCost
  const heldRevenue = controls.heldRevenue * (1 - inputs.clearRate) * inputs.heldInflowMultiplier
  const heldCost = controls.heldCost * inputs.costMultiplier
  const totalContribution = deliveredContribution + heldRevenue * inputs.recoveryRate - heldCost
  const unservedTonnes = controls.heldTonnes * (1 - inputs.clearRate) * inputs.heldInflowMultiplier
  const difot = Math.min(100, Math.max(0, controls.postDIFOT * inputs.serviceMultiplier))
  const availableRoutes = routes.filter((route) => route.kind === 'delivered' && !inputs.unavailableRoutes.includes(route.name))
  const availableTonnes = availableRoutes.reduce((sum, route) => sum + route.tonnes, 0)
  const routeAllocation = availableRoutes.map((route) => ({ route: route.name, tonnes: route.tonnes, share: availableTonnes ? route.tonnes / availableTonnes * 100 : 0 }))
  const triggerCrossings = []
  if (heldRevenue > controls.contractedRevenue * 0.05) triggerCrossings.push('Held revenue >5% of portfolio')
  if (difot < 90) triggerCrossings.push('DIFOT <90%')
  if (totalContribution < 0) triggerCrossings.push('Prospective contribution negative')
  if (unservedTonnes > controls.totalTonnes * 0.1) triggerCrossings.push('Unserved tonnes >10% of portfolio')
  const cellTriggerCrossings = cells.reduce((result, cell) => {
    if (result.length >= 6) return result
    if (cell.kind === 'held') {
      if (cell.heldRevenue * (1 - inputs.clearRate) * inputs.heldInflowMultiplier > controls.contractedRevenue * 0.01) result.push(`${cell.customer} × ${cell.product} × Held: open revenue trigger`)
      return result
    }
    const cellCost = (cell.cost - cell.insurance - cell.penalty) * inputs.costMultiplier + cell.insurance * inputs.insuranceMultiplier + cell.penalty * inputs.penaltyMultiplier
    if (cell.recognized + cell.adverse * inputs.recoveryRate - cellCost < 0) result.push(`${cell.customer} × ${cell.product} × ${cell.route}: negative contribution`)
    return result
  }, [])
  return {
    totalContribution,
    ...
    difot,
    recoveredSurcharge,
    optionUtilization: controls.deliveredTonnes ? availableTonnes / controls.deliveredTonnes * 100 : 0,
    triggerCrossings,
    cellTriggerCrossings,
    routeAllocation,
  }
}
...
function CustomerPareto({ activeCustomer, setActiveCustomer }) {
  ...
  const topThree = visibleCustomers.slice(0, 3).reduce((sum, customer) => sum + customer.adverse, 0)
  const chartData = visibleCustomers.map((customer, rank) => ({ ...customer, rank }))

  return (
    <article className="panel pareto-panel"><div className="panel-heading"><div><div className="eyebrow">commercial transmission</div><h3>Where the dollars land</h3></div><span className="panel-tag">top 5 / {customers.length} accounts</span></div><div className="pareto-chart"><VisXYContainer ariaLabel="Top five customers by positive adverse sensitivity" className="unovis-chart" data={chartData} height={250} margin={{ top: 8, right: 20, bottom: 35, left: 112 }} xDomain={[0, max * 1.1]} yDomain={[-0.5, chartData.length - 0.5]}><VisGroupedBar color="#1a211e" groupPadding={0.18} orientation="horizontal" roundedCorners={false} x={(customer) => customer.rank} y={(customer) => customer.adverse} /><VisAxis domainLine={false} gridLine label="positive adverse $" labelColor="#606562" labelFontSize="10px" tickFormat={(tick) => money(tick, 0)} tickTextColor="#606562" tickTextFontSize="10px" type="x" /><VisAxis domainLine={false} gridLine={false} tickFormat={(tick) => chartData[Math.round(Number(tick))]?.name ?? ''} tickTextColor="#606562" tickTextFontSize="10px" tickValues={chartData.map((customer) => customer.rank)} type="y" /><VisTooltip className="unovis-tooltip" triggers={{ [VisGroupedBarSelectors.bar]: (customer) => `<strong>${customer.name}</strong><span>${money(customer.adverse)} positive sensitivity</span><span>${percent(customer.fullShare)} portfolio share</span>` }} /></VisXYContainer></div><div className="pareto-list">{visibleCustomers.map((customer, index) => { const cumulative = visibleCustomers.slice(0, index + 1).reduce((sum, item) => sum + item.adverse, 0); const cumulativeShare = (cumulative / total) * 100; const active = activeCustomer === customer.name; return <button className={`pareto-row ${active ? 'selected' : ''}`} key={customer.name} onClick={() => setActiveCustomer(active ? '' : customer.name)} type="button"><span className="rank">{String(index + 1).padStart(2, '0')}</span><span className="pareto-main"><span className="pareto-label"><b>{customer.name}</b><em>{percent(customer.fullShare)} portfolio share</em></span></span><span className="pareto-value">{money(customer.adverse)}<small>{percent(cumulativeShare)} cum.</small></span></button> })}</div><div className="pareto-callout"><Icon name="target" size={16} /> Top three capture {percent(topThree / total * 100)} of the full shock sensitivity denominator.</div></article>
  )
}
...
function VulnerabilityMap({ activeCustomer, setActiveCustomer }) {
  ...
  return (
    <article className="panel map-panel"><div className="panel-heading"><div><div className="eyebrow">impact × rate severity</div><h3>Scale does not equal severity</h3></div><span className="panel-tag">hover / click a point</span></div><div className="map-legend"><span><i className="legend-dot blue" /> positive adverse sensitivity $</span><span><i className="legend-dot amber" /> sensitivity / shock revenue</span></div><div className="scatter-chart"><VisXYContainer ariaLabel="Customer impact versus rate severity map" className="unovis-chart" data={chartData} height={280} margin={{ top: 22, right: 20, bottom: 42, left: 72 }} xDomain={[0, 400]} yDomain={[0, max * 1.15]}><VisScatter color={(customer) => customer.name === activeCustomer ? '#cc2e39' : '#1a211e'} events={{ [VisScatterSelectors.point]: { click: (customer) => setActiveCustomer(customer.name === activeCustomer ? '' : customer.name) } }} label={(customer) => shortName(customer.name)} labelColor="#606562" labelHideOverlapping={false} labelPosition="top" shape="circle" size={(customer) => customer.contracted} sizeRange={[12, 36]} strokeColor={(customer) => customer.name === activeCustomer ? '#cc2e39' : '#1a211e'} strokeWidth={1} x={(customer) => customer.severity} y={(customer) => customer.adverse} /><VisAxis domainLine={false} gridLine={false} label="rate severity" labelColor="#606562" labelFontSize="10px" tickFormat={(tick) => `${Number(tick).toFixed(0)}%`} tickTextColor="#606562" tickTextFontSize="10px" type="x" /><VisAxis domainLine={false} gridLine label="positive adverse $" labelColor="#606562" labelFontSize="10px" tickFormat={(tick) => money(tick, 0)} tickTextColor="#606562" tickTextFontSize="10px" type="y" /><VisTooltip className="unovis-tooltip" triggers={{ [VisScatterSelectors.point]: (customer) => `<strong>${customer.name}</strong><span>${money(customer.adverse)} positive sensitivity</span><span>${percent(customer.severity)} severity · ${money(customer.contracted)} contracted</span>` }} /></VisXYContainer></div><div className="map-footer"><span>Bubble size = full contracted revenue</span><span>Axes use raw dollars and aggregate rates</span></div></article>
  )
}
...
function ScenarioPanel({ activeScenario, setActiveScenario, scenarioInputs, setScenarioInputs }) {
  ...
  const output = calculateScenario(inputs)
  const triggers = [...output.triggerCrossings, ...output.cellTriggerCrossings].slice(0, 8)
  const fields = [
  ...
  return (
    <article className="panel scenario-panel"><div className="panel-heading"><div><div className="eyebrow">stage 07 / owner-editable decision stress</div><h3>Three scenarios. Numeric gates.</h3></div><span className="panel-tag">browser inputs / not yet live</span></div><div className="scenario-grid">{scenarios.map((scenario, index) => <button className={`scenario-card ${scenario.tone} ${activeScenario === index ? 'active' : ''}`} key={scenario.name} onClick={() => setActiveScenario(index)} type="button"><span className="scenario-index">0{index + 1}</span><b>{scenario.name}</b><p>{scenario.detail}</p><span className="scenario-action">{scenario.action}</span></button>)}</div><div className="scenario-input-panel"><div className="scenario-input-heading"><div><span className="eyebrow">owner inputs / {selected.name}</span><p>Replace proposal defaults with approved quotes, capacity and recovery terms.</p></div><button className="text-button" onClick={resetInputs} type="button">Reset proposal defaults</button></div><div className="scenario-inputs">{fields.map(([key, label, owner, min, max, step]) => <label key={key}><span>{label}<small>{owner}</small></span><input max={max} min={min} onChange={(event) => updateInput(key, Number(event.target.value))} step={step} type="number" value={inputs[key]} /></label>)}</div><div className="scenario-routes"><span>unavailable routes <small>Network Planning</small></span>{deliveredRoutes.map((route) => <label key={route.name}><input checked={inputs.unavailableRoutes.includes(route.name)} onChange={() => updateInput('unavailableRoutes', inputs.unavailableRoutes.includes(route.name) ? inputs.unavailableRoutes.filter((item) => item !== route.name) : [...inputs.unavailableRoutes, route.name])} type="checkbox" />{route.name}</label>)}</div></div><div className={`scenario-detail ${selected.tone}`}><div><span className="eyebrow">computed output / {selected.name}</span><div className="scenario-output-grid"><span><small>total contribution</small><b>{money(output.totalContribution)}</b></span><span><small>Held revenue</small><b>{money(output.heldRevenue)}</b></span><span><small>unserved tonnes</small><b>{Math.round(output.unservedTonnes).toLocaleString()}</b></span><span><small>DIFOT</small><b>{percent(output.difot)}</b></span></div></div><span className="scenario-detail-action"><Icon name="arrow" size={16} /> {selected.action}</span></div><div className="scenario-subgrid"><div><small>route allocation</small><span>{output.routeAllocation.map((route) => <em key={route.route}>{route.route}: {percent(route.share)} ({Math.round(route.tonnes).toLocaleString()}t)</em>)}</span></div><div><small>account / cell triggers</small><span>{triggers.length ? triggers.map((trigger) => <em className="trigger-chip" key={trigger}>{trigger}</em>) : <em className="clear-chip">none</em>}</span></div><div><small>computed drivers / capacity</small><span><em>recovery {percent(inputs.recoveryRate * 100)} / {money(output.recoveredSurcharge)}</em><em>available route tonnes {percent(output.optionUtilization)}</em>{inputs.unavailableRoutes.map((route) => <em key={route}>unavailable: {route}</em>)}</span></div></div><div className="scenario-note">{selected.assumptionSource} Owners: {selected.inputOwner}.</div></article>
  )
}
  ...
  return (
    <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark">S</span><div><strong>SOMAIYA</strong><small>network control</small></div></div><div className="sidebar-status"><span className="live-dot" /> War-room snapshot <b>R2</b></div><nav className="side-nav" aria-label="Dashboard sections"><a className="active" href="#command"><Icon name="compass" /> Command center</a><a href="#exposure"><Icon name="chart" /> Exposure map</a><a href="#queue"><Icon name="truck" /> Held queue <span>{controls.heldRows}</span></a><a href="#decisions"><Icon name="flag" /> Decisions <span>05</span></a></nav><div className="sidebar-bottom"><div className="source-card"><div className="eyebrow">evidence base</div><strong>Cleaned workbook</strong><span>{metadata.sourceRows} canonical rows</span><span>{dateLabel(metadata.observationStart)} — {dateLabel(metadata.observationEnd)}</span><small>Provisional control set</small></div><div className="sidebar-footer"><span>QUANTIZ’26</span><span>generated / internal</span></div></div></aside>
      <main className="main-content"><header className="topbar"><div className="crumb"><span>R2</span><i>/</i> STRAIT OUTTA HORMUZ</div><div className="topbar-right"><span className="refresh"><i /> snapshot {dateLabel(metadata.asOf)}</span><label className="focus-select"><span>focus</span><select aria-label="Focus account" onChange={(event) => setActiveCustomer(event.target.value)} value={activeCustomer}><option value="">Portfolio view</option>{customers.map((customer) => <option key={customer.name} value={customer.name}>{customer.name}</option>)}</select></label></div></header><div className="provisional-banner"><span><Icon name="lock" size={15} /> PROVISIONAL — {metadata.sourceRows}-row cleaned file</span><button onClick={() => setShowEvidence(!showEvidence)} type="button">{showEvidence ? 'Hide' : 'Open'} source controls <Icon name="arrow" size={14} /></button></div>{showEvidence && <div className="evidence-drawer"><span><b>Source gate</b> case states {metadata.caseStatedRows} rows; supplied workbook has {metadata.sourceRows} unique IDs.</span><span><b>Safe treatment</b> no prorating, no manufactured rows, no raw concentration denominator.</span><span><b>Refresh gate</b> analysis → QA → build; conflicting duplicate IDs fail the refresh.</span></div>}
        ...
        <section className="section-pad"><SectionHeader eyebrow="07 / next disruption" title="Stress the decision, not the spreadsheet." copy="The displayed inputs are explicit proposal assumptions; outputs recalculate from the workbook controls." /><ScenarioPanel activeScenario={activeScenario} scenarioInputs={scenarioInputs} setActiveScenario={setActiveScenario} setScenarioInputs={setScenarioInputs} /></section>
